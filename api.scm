       ...
       (acknowledgers
        ...
        ((accidental-interface engraver grob source-engraver)
         (let ((musicexport (ly:context-property context ctprop::music-export))
              ...
              (moment (ly:context-property context 'measurePosition (ly:make-moment 0)))
              (altered-pitch (ly:music-property (grob-cause grob) 'pitch))
              (accidental (accidental-interface::calc-alteration grob)))
           (let ((pitch-acc (tree-get musicexport (list bar moment staff-id voice-id 'pitch-acc))))
             (tree-set! musicexport (list bar moment staff-id voice-id 'pitch-acc)
                        (cons (list altered-pitch accidental) (if pitch-acc pitch-acc '())))
             )))
