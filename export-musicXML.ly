...
#(define-public (exportMusicXML musicexport filename . options)
   (let ((grid (tree-create 'grid))
     ...
     (define (writeduration dur)
       (if (ly:duration? dur)
           (let* ((divlen (duration-factor dur)))
             ;(ly:message "dur: ~A" (* divlen divisions))
             (writeln "<duration>~A</duration>" (* divlen divisions))
             )))
                         ...
                         (let ((music (tree-get musicexport (list measure moment staff voice))))
                           (if (ly:music? music)
                               (let ((dur (ly:music-property music 'duration))
                                 ...
                                 ; TODO staff grouping!
                                 (writemusic music 1 voice `(beam . ,(cond ((eq? 'start beam) 'begin)((symbol? beam) beam)((symbol? beamcont) beamcont))))
                                 (if (ly:duration? dur)
